...
public final class DiagnosticCrashReporter: NSObject, MXMetricManagerSubscriber, @unchecked Sendable {
  ...
  public func didReceive(_ payloads: [MXMetricPayload]) {
    ...
    DiagnosticLog.shared.log(.lifecycle, "MetricKit: received \(payloads.count) metric payload(s)")
  ...
  private func prune(prefix: String, keep: Int) {
  ...
  private func write(_ data: Data, prefix: String, index: Int) {
    ...
    do {
      ...
      os_log("MetricKit payload written: %{public}@", log: log, type: .info, url.lastPathComponent)
    } catch {
      os_log("MetricKit payload write failed: %{public}@", log: log, type: .error, error.localizedDescription)
    }
  }
}
