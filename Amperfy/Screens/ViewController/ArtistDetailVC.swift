...
class ArtistDetailVC: MultiSourceTableViewController {
  ...
  override func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    if abs(scrollView.contentOffset.y - cassetteNavLastY) > 8 {
      ...
      print("CASSETTE-NAV: artist off.y=\(Int(scrollView.contentOffset.y)) adjTop=\(scrollView.adjustedContentInset.top) adjBot=\(scrollView.adjustedContentInset.bottom) atTop=\(atTop) drag=\(dir) tracking=\(scrollView.isTracking)")
    }
  }
